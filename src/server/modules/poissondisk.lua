...
function generate_poisson(width, height, min_dist, new_points_count)
	...
	while not RandomQueue.empty(processList) do
		...
		for i = 0, new_points_count do
			...
			-- to other points
			if inRectangle(newPoint, width, height) and not inNeighbourhood(grid, newPoint, min_dist, cellSize) then
...
function generateRandomPointAround(point, mindist)
	...
	local newX = point.x + radius * math.cos(angle)
	local newY = point.y + radius * math.sin(angle)
	return Point.new(newX, newY)
end

function inNeighbourhood(grid, point, mindist, cellSize)
	local gridPoint = imageToGrid(point, cellSize)
	local cellsAroundPoint = squareAroundPoint(grid, gridPoint, 5)
	for _, cell in pairs(cellsAroundPoint) do
...
function squareAroundPoint(grid, gridPoint, n)
	...
	for i = -extreme, extreme do
		for j = -extreme, extreme do
			local ii = i + gridPoint.x
			local jj = j + gridPoint.y
			local data = grid[gridToString(Point.new(ii, jj))]
...
local seeds = {
	Globals.worldGen.SEED + 3,
	math.floor(Globals.worldGen.SEED + 10 / 2),
	Globals.worldGen.SEED + 50,
	Globals.worldGen.SEED + 66,
}
...
function poisson_points_list(width, height, mindist, add_points)
	math.randomseed(seeds[math.random(1, #seeds)])
